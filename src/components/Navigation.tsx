  ...
  return (
    <>
      ...
      <header className="md:hidden bg-[#0A0A0A]/95 backdrop-blur-md border-b border-white/10 flex justify-between items-center w-full px-4 h-16 fixed top-0 left-0 z-40">
        <div className="flex items-center gap-2">
          <button
            ...
            aria-label="Search"
            className="text-[#ffb59a] hover:text-[#FF5C00] transition-colors p-1.5 flex items-center justify-center rounded-lg bg-neutral-900/60 border border-neutral-800"
          >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="p-1.5 rounded-lg bg-[#FF4D00]/10 hover:bg-[#FF4D00]/20 text-[#FF6B26] border border-[#FF4D00]/30 transition-colors flex items-center justify-center"
          >
          ...
          {security.isLockEnabled && (
            <button
              ...
              className="p-1.5 rounded-lg bg-neutral-900 hover:bg-neutral-800 text-amber-400 border border-neutral-800 transition-colors"
            >
          ...
          <button
            onClick={onOpenProfile || (() => onSelectTab('recap'))}
            className="w-8 h-8 rounded-full bg-[#1A1A1A] border border-white/10 overflow-hidden flex-shrink-0 focus:outline-none focus:ring-2 focus:ring-[#FF5C00]"
      ...
      <aside className="hidden md:flex flex-col fixed left-0 top-0 h-full w-64 bg-[#140b07] border-r border-white/10 p-5 z-40 overflow-y-auto">
        ...
        <div className="mb-5 p-3 rounded-xl bg-gradient-to-br from-[#1E110A] to-[#2C140A] border border-[#FF4D00]/30 shadow-lg shadow-black/40">
          ...
          <button
            onClick={onOpenPortfolio}
            className="w-full py-1.5 px-2.5 bg-[#FF4D00]/20 hover:bg-[#FF4D00]/30 text-[#FF8246] border border-[#FF4D00]/40 rounded-lg text-xs font-semibold transition-colors flex items-center justify-center gap-1.5"
          >
        ...
        <nav className="flex flex-col gap-1.5 flex-grow">
          <button
            onClick={() => onSelectTab('home')}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-semibold transition-all text-left ${
              currentTab === 'home'
          ...
          <button
            onClick={() => onSelectTab('people')}
            className={`flex items-center justify-between px-3 py-2.5 rounded-lg text-xs font-semibold transition-all text-left ${
              currentTab === 'people'
                ? 'bg-[#FF5C00] text-black font-bold shadow-md'
                : 'text-[#e4beb1] hover:text-white hover:bg-[#2c1c16]'
            }`}
          >
            ...
            {overdueCount > 0 && (
              <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-[#370e00] text-[#ffb59a] border border-[#FF5C00]/40 font-bold">
                {overdueCount}
              </span>
            )}
          </button>
          ...
          <div className="pt-3 mt-2 border-t border-white/10 space-y-1">
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-medium text-neutral-400 hover:text-rose-300 hover:bg-rose-950/20 transition-colors text-left"
            >
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-medium text-neutral-400 hover:text-amber-300 hover:bg-amber-950/20 transition-colors text-left"
            >
        ...
        <div className="pt-3 border-t border-white/10 mt-auto">
          <div
            onClick={onOpenProfile || (() => onSelectTab('recap'))}
            className="flex items-center gap-2.5 cursor-pointer p-2 rounded-xl hover:bg-white/5 transition-colors group"
            title="Edit My Profile & Picture"
          >
            <div className="w-10 h-10 rounded-full border border-[#FF4D00]/50 group-hover:border-[#FF5C00] overflow-hidden bg-[#1A1A1A] transition-colors relative flex-shrink-0">
              <img
                alt={profile.name}
                className="w-full h-full object-cover"
                src={profile.avatarUrl}
